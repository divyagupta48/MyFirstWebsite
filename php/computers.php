...
$sql="select * from created where category='computer'";
...
$sql="select * from starred where category='computer'";
$result=mysqli_query($con, $sql);
while($row=mysqli_fetch_array($result))
{
	echo "<tr><td>$row[topic_name]</td><td>&nbsp</td><td>$row[date_created]</td><td>&nbsp</td><td>$row[description]</td><td>&nbsp</td>";
	echo "<td><td>&nbsp&nbsp&nbsp&nbsp&nbsp</td><td><form action='chat.php' method='get'>
	<input type='hidden' name='var' value=$row[topic_id]>
	<input type='submit' name=$row[topic_id] value='View Discussion'>
	<input type='button' class=st value='Mark Star' onclick='star();'></form></td></tr>";
}
echo "</table>";
	?>
