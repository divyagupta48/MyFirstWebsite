...
$sql="select * from created where category='debate'";
...
$sql="select * from starred where category='debate'";
$result=mysqli_query($con, $sql);
while($row=mysqli_fetch_array($result))
{
	echo "<tr><td>$row[topic_name]</td><td>&nbsp</td><td>$row[date_created]</td><td>&nbsp</td><td>$row[description]</td><td>&nbsp</td>";
	echo "<td><td>&nbsp&nbsp&nbsp&nbsp&nbsp</td><td><form action='chat.php' method='get'><input type='hidden' name='var' value=$row[topic_id]>
